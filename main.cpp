...
#define ACC_OF_GRAVITY -1.0f //-9.81f
...
const char  SPRITESHEET_FILEPATH[] = "assets/goku2.png",
PLATFORM_FILEPATH[] = "assets/platformPack_tile027.png",
SPRITE_FILEPATH[] = "assets/dragonball.png",
...
void initialise()
{
    ...
    g_game_state.player->type = PLAYER;

    // Walking
    g_game_state.player->m_walking[g_game_state.player->LEFT]   = new int[3] { 4, 5, 6 };
    g_game_state.player->m_walking[g_game_state.player->RIGHT]  = new int[3] { 7, 8, 9 };
    g_game_state.player->m_walking[g_game_state.player->UP]     = new int[3] { 10, 11, 12 };
    ...
    {   
        g_game_state.platforms[i].type = PLATFORM;
    ...
    //DRAGONBALL
    g_game_state.balls = new Entity[BALL_COUNT];

    for (int i = 0; i < BALL_COUNT; i++)
    {   
        g_game_state.balls[i].type = BALL;
        g_game_state.balls[i].m_texture_id = load_texture(SPRITE_FILEPATH);
        g_game_state.balls[i].set_position(glm::vec3(i - 3.0f, -2.0f, 0.0f));
        g_game_state.balls[i].update(0.0f, NULL, 0);
    }
    //g_game_state.balls[0].set_position(glm::vec3(5.0f, -2.0f, 0.0f));
...
void update()
{
    ...
    {  
        ...
        Entity *collidable = new Entity[PLATFORM_COUNT + BALL_COUNT];
        for (int i = 0; i < PLATFORM_COUNT; i++)
        {
            collidable[i] = g_game_state.platforms[i];
        }
        for (int i = PLATFORM_COUNT; i < BALL_COUNT + PLATFORM_COUNT; i++)
        {
            collidable[i] = g_game_state.balls[i - PLATFORM_COUNT];
        }
        g_game_state.player->update(FIXED_TIMESTEP, collidable, PLATFORM_COUNT + BALL_COUNT);
        delta_time -= FIXED_TIMESTEP;
    }

    EntityType collided = g_game_state.player->collided;
    if (collided == BALL)
    {
        win = true;
    }
    else if (collided == PLATFORM)
    {
        lose = true;
    }
...
void render()
{
    ...
    //BALL
    for (int i = 0; i < BALL_COUNT; i++) g_game_state.balls[i].render(&g_shader_program);

    GLuint g_font_id = load_texture(FONT_FILEPATH);

    if (win and !lose)
    {
        draw_text(&g_shader_program, g_font_id, "Mission Accomplished!", 0.4, 0.01f, glm::vec3(-3.0f, 0.0f, 0));
    }

    if (lose and !win)
    {
        draw_text(&g_shader_program, g_font_id, "Mission Failed!", 0.4, 0.01f, glm::vec3(-3.0f, 0.0f, 0));
    }
